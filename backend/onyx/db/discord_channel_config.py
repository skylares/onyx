...
def _build_persona_name(channel_name: str) -> str:
    return f"{DISCORD_BOT_PERSONA_PREFIX}{channel_name}"
...
def create_discord_channel_persona(
    db_session: Session,
    channel_name: str,
    ...
    # create/update persona associated with the Discord channel
    persona_name = _build_persona_name(channel_name)
    default_prompt = get_default_prompt(db_session)
    persona = upsert_persona(
        user=None,  # Discord channel Personas are not attached to users
...
def insert_discord_channel_config(
    db_session: Session,
    discord_bot_id: int,
    persona_id: int | None,
    ...
    discord_channel_config = DiscordChannelConfig(
        discord_bot_id=discord_bot_id,
        persona_id=persona_id,
        channel_config=channel_config,
        enable_auto_filters=enable_auto_filters,
    )
    db_session.add(discord_channel_config)
    db_session.commit()

    return discord_channel_config
...
def update_discord_channel_config(
    ...
    discord_channel_config = db_session.scalar(
        select(DiscordChannelConfig).where(
            DiscordChannelConfig.id == discord_channel_config_id
        )
    )
    if discord_channel_config is None:
        raise ValueError(
            f"Unable to find Discord channel config with ID {discord_channel_config_id}"
        )

    # get the existing persona id before updating the object
    existing_persona_id = discord_channel_config.persona_id

    # update the config
    # NOTE: need to do this before cleaning up the old persona or else we
    # will encounter `violates foreign key constraint` errors
    discord_channel_config.persona_id = persona_id
    discord_channel_config.channel_config = channel_config
    discord_channel_config.enable_auto_filters = enable_auto_filters
    ...
    if persona_id != existing_persona_id and existing_persona_id:
        ...
        # if the existing persona was one created just for use with this Discord channel,
        # then clean it up
        if existing_persona and existing_persona.name.startswith(
            DISCORD_BOT_PERSONA_PREFIX
        ):
...
def remove_discord_channel_config(
    db_session: Session,
    discord_channel_config_id: int,
    user: User | None,
) -> None:
    ...
    db_session.delete(discord_channel_config)
    db_session.commit()


def fetch_discord_channel_configs(
    db_session: Session, discord_bot_id: int | None = None
) -> Sequence[DiscordChannelConfig]:
    if not discord_bot_id:
        return db_session.scalars(select(DiscordChannelConfig)).all()

    return db_session.scalars(
        select(DiscordChannelConfig).where(
            DiscordChannelConfig.discord_bot_id == discord_bot_id
        )
    ).all()


def fetch_discord_channel_config(
    db_session: Session, discord_channel_config_id: int
