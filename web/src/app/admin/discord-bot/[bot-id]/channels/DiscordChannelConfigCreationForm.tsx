...
import { DocumentSet, DiscordChannelConfig } from "@/lib/types";
import {
  BooleanFormField,
  Label,
  SelectorFormField,
...
export const DiscordChannelConfigCreationForm = ({
  discord_bot_id,
  documentSets,
  personas,
  existingDiscordChannelConfig,
}: {
  ...
  const isUpdate = existingDiscordChannelConfig !== undefined;
  const { popup, setPopup } = usePopup();
  const router = useRouter();
  const existingDiscordBotUsesPersona = existingDiscordChannelConfig?.persona
    ? !isPersonaADiscordBotPersona(existingDiscordChannelConfig.persona)
    : false;
  const [usingPersonas, setUsingPersonas] = useState(
    existingDiscordBotUsesPersona
  );
      ...
      <CardSection>
        ...
        <Formik
          initialValues={{
            ...
            channel_name:
              existingDiscordChannelConfig?.channel_config.channel_name,
            answer_validity_check_enabled: (
              existingDiscordChannelConfig?.channel_config?.answer_filters || []
            ).includes("well_answered_postfilter"),
            questionmark_prefilter_enabled: (
              existingDiscordChannelConfig?.channel_config?.answer_filters || []
            ).includes("questionmark_prefilter"),
            respond_mention_only:
              existingDiscordChannelConfig?.channel_config
                ?.respond_mention_only || false,
            respond_to_bots:
              existingDiscordChannelConfig?.channel_config?.respond_to_bots ||
              false,
            ...
            enable_auto_filters:
              existingDiscordChannelConfig?.enable_auto_filters || false,
            respond_member_group_list:
              existingDiscordChannelConfig?.channel_config?.allowed_role_ids ??
              [],
            still_need_help_enabled:
              existingDiscordChannelConfig?.channel_config?.follow_up_tags !==
              undefined,
            follow_up_tags:
              existingDiscordChannelConfig?.channel_config?.follow_up_tags,
            document_sets:
              existingDiscordChannelConfig &&
              existingDiscordChannelConfig.persona
                ? existingDiscordChannelConfig.persona.document_sets.map(
            ...
            persona_id:
              existingDiscordChannelConfig?.persona &&
              !isPersonaADiscordBotPersona(existingDiscordChannelConfig.persona)
                ? existingDiscordChannelConfig.persona.id
                : knowledgePersona?.id ?? null,
          }}
          validationSchema={Yup.object().shape({
            discord_bot_id: Yup.number().required(),
            channel_name: Yup.string(),
            answer_validity_check_enabled: Yup.boolean().required(),
            questionmark_prefilter_enabled: Yup.boolean().required(),
            respond_mention_only: Yup.boolean().required(),
          ...
          onSubmit={async (values, formikHelpers) => {
            ...
            let response;
            if (isUpdate) {
              response = await updateDiscordChannelConfig(
                existingDiscordChannelConfig.id,
                cleanedValues
              );
            } else {
              response = await createDiscordChannelConfig(cleanedValues);
            }
            formikHelpers.setSubmitting(false);
            if (response.ok) {
              router.push(`/admin/discord-bot/${discord_bot_id}`);
            } else {
                      ...
                      {values.still_need_help_enabled && (
                        <CollapsibleSection prompt="Configure Still Need Help Button">
                          <TextArrayField
                            ...
                            subtext={
                              <div>
                                The Discord users / groups we should tag if the
